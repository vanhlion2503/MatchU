...
function requireAnyPermission(...permissions) { return (req, res, next) => req.admin?.role === 'super_admin' || permissions.some((permission) => req.admin?.permissions?.includes(permission)) ? next() : forbidden(req, res); }
