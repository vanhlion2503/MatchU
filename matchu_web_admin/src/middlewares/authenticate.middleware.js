...
module.exports = async (req, res, next) => {
  ...
  if (canReadReports && req.method === 'GET') {
    try {
      res.locals.pendingReportCount = await getPendingReportCount();
    } catch (_) {
      // The navigation remains usable when the optional counter cannot load.
    }
  }
  return next();
};
